...
4. **`src/shop_bot/factory_bot/middleware.py`** — задача 3
   ...
   - `OwnerCabinetEnhanceMiddleware` добавляет кнопку «🗑 Удалить моего бота» в личный кабинет клона сразу под «Вывод средств».
...
### Кабинет владельца
- В клоне у владельца в личном кабинете под «Вывод средств» есть «🗑 Удалить моего бота» (удаляется только текущий клон).

...
Модули можно загружать в виде ZIP архивов прямо из админ-панели без перезапуска приложения.

### Способ 1: Через веб-панель

1. Войдите в админ-панель (`https://ваш-домен/`)
2. Перейдите в раздел **🧩 Модули**
3. В поле **📦 Загрузить новый модуль** выберите ZIP архив
4. Нажмите **Загрузить и установить**

ZIP архив должен содержать модуль со следующей структурой:
```
my_module.zip
  my_module/
    __init__.py
...
⚠️ **Важно**: ZIP должен содержать директорию модуля, а не файлы напрямую.

### Способ 2: Через Telegram-админку

```
/admin → Модули → выбрать модуль → Включить
```
...
ok, message = loader.import_module_from_zip(
    "/path/to/module.zip",
    auto_enable=True  # Автоматически включить модуль
)
print(message)
```

### Преимущества загрузки ZIP

 ...
 * from the URL. store.js and domain scripts load before this file.
 *
 * Logical splits for later PRs (helpers also live in sibling files):
 *   telegram.js     — Bot API version / appearance / haptic
 *   ui.js           — toast / popup / notification
 *   transactions.js — история операций на «Финансы» (грузится до этого файла)
 *   keys-page.js    — табы/поиск/пагинация ключей и подарки (грузится до этого файла)
 */
...
function updateThemeColor() {
    ...
    if (meta.content !== hex) {
        meta.content = hex;
        safeTelegramAppearance({ header: hex });
    }
}
...
function supportFileUrl(messageId) {
    return '/api/support/ticket-file/' + encodeURIComponent(messageId);
}
...
// Accordion (.key-toggle) — в keys-page.js. Save/delete заметки здесь:
// делегирование, без повторного forEach после search/gifts и без window.toggleKeyCard.
...
// Leftover #152: универсальная модалка и копирование —
// data-action-modal, data-copy-action (текст из соседнего .font-mono,
// не из data-*). SSR-карточки ключей — data-key-action renew/auto-renew/
// lte-topup; планы — .plan-btn. data-tx-action — в transactions.js,
// data-gifts-page / data-profile-keys-page / data-gift-action — в keys-page.js.
document.addEventListener('click', (event) => {
    ...
    if (action === 'copy-key') {
        ...
        openActionModal('devices', keyId, host);
    } else if (action === 'rename' && keyId) {
...
let _paymentPollOnProcessing = null;
let _paymentPollProcessingShown = false;
const PAYMENT_POLL_MIN_MS = 8000;
const PAYMENT_POLL_MAX_MS = 20000;
const PAYMENT_POLL_MAX_WAIT_MS = 40 * 60 * 1000;
let activePaymentId = null;
const RENDERED_USER_ID = parseInt(String(window.RENDERED_USER_ID || 0), 10) || 0;
...
async function openLteTopup(keyId) {
    ...
    try {
        const resp = await window.apiFetch('/api/lte-packages?key_id=' + encodeURIComponent(keyId));
        const data = await resp.json();
        if (!data.ok) {
...
async function openMethodsList() {
    ...
    try {
        ...
        const response = await fetch('/api/payment-methods', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_id: userId, token: window.getAuthToken() || '', init_data: window.getTgInitData() })
        });
        const data = await response.json();
        if (data.ok) {
    ...
    try {
        ...
        const response = await fetch(isLte ? '/api/create-lte-topup-payment' : '/api/create-payment', {
            ...
            body: JSON.stringify(isLte ? {
                token: window.getAuthToken() || '',
                init_data: window.getTgInitData(),
...
function _stopStatusPolling() {
    ...
    _paymentPollOnProcessing = null;
    _paymentPollProcessingShown = false;
...
async function _tickPaymentPoll() {
    ...
    try {
        ...
        // Оплата уже принята, но услуга ещё выдаётся: /api/check-payment
        // отвечает processing и держит paid: false. Без этой ветки окно
        // продолжало бы писать «ожидаем оплату», как будто платежа не было.
        if (data.ok && data.processing && _paymentPollOnProcessing && !_paymentPollProcessingShown) {
            _paymentPollProcessingShown = true;
            _paymentPollOnProcessing(data);
        }
    } catch (e) {
...
function startStatusPolling(pid, onPaid, onProcessing) {
    if (!pid) return;
    _stopStatusPolling();
    _paymentPollStopped = false;
    _paymentPollPid = pid;
    _paymentPollOnPaid = onPaid || (async (data) => { showSuccessScreen(data); });
    _paymentPollOnProcessing = onProcessing || null;
...
async function openActionModal(type, keyId, extraData = '') {
    ...
    if (type === 'devices') {
        ...
        const storedCommentEl = document.getElementById('comment-text-' + keyId);
        const storedComment = storedCommentEl ? storedCommentEl.textContent : '';
        ...
        contentEl.innerHTML = `
            <div class="flex flex-col gap-3">
                ...
                <textarea id="action-comment-input" style="outline: none !important; box-shadow: none !important; -webkit-tap-highlight-color: transparent !important; -webkit-appearance: none; border-color: rgba(255,255,255,0.1) !important;" class="w-full h-28 bg-black/40 border border-white/10 rounded-xl p-3 text-xs text-white placeholder-gray-600 focus:outline-none focus:ring-0 focus:ring-transparent focus:border-white/10 transition-colors resize-none" placeholder="Введите заметку к ключу..."></textarea>
...
async function saveComment(keyId, isDelete = false) {
    ...
    const comment = isDelete ? "" : (inputEl ? inputEl.value : "");
...
openActionModal = async function (type, keyId, extraData = '') {
    if (type === 'rename') {
        ...
        const nameCard = document.querySelector('[data-key-id="' + String(keyId) + '"]');
        const currentName = nameCard ? (nameCard.getAttribute('data-key-name') || '') : (extraData || '');
        contentEl.innerHTML = `
            <div class="flex flex-col gap-3">
                ...
                <div id="action-rename-counter" class="text-[9px] text-gray-600 text-right px-1">0/30</div>
                <div class="flex gap-2 w-full">
                    <button id="action-rename-delete-btn" class="w-12 bg-red-500/10 text-red-500 py-2.5 rounded-xl flex items-center justify-center hover:bg-red-500/20 active:scale-[0.98] transition-all shrink-0">
                        <span class="material-symbols-rounded text-sm">delete</span>
                    </button>
                    <button id="action-rename-btn"
                        class="flex-1 bg-white text-black py-2.5 rounded-xl font-bold text-[10px] uppercase tracking-wider shadow-[0_4px_15px_rgba(255,255,255,0.1)] hover:shadow-[0_6px_20px_rgba(255,255,255,0.2)] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                        <span class="material-symbols-rounded text-sm">save</span>
                        Сохранить
                    </button>
        ...
        if (inp) {
            ...
            if (counter) counter.textContent = inp.value.length + '/30';
            inp.addEventListener('input', () => { counter.textContent = inp.value.length + '/30'; });
            inp.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') { e.preventDefault(); renameKey(keyId); }
            });
            document.getElementById('action-rename-delete-btn')?.addEventListener('click', () => renameKey(keyId, true));
            document.getElementById('action-rename-btn')?.addEventListener('click', () => renameKey(keyId));
...
function _payoutDom(tag, className, text) {
...
function _payoutMethodLabel(m) {
    const type = m && _M_LABELS[m.method_type] ? m.method_type : '';
    const typeLabel = _M_LABELS[type] || 'Метод';
    const bank = m && m.bank_name ? String(m.bank_name) : '';
    return bank ? (typeLabel + ' · ' + bank) : typeLabel;
}

function _payoutRequisiteText(m) {
    if (m && m.requisite_masked) return String(m.requisite_masked);
    return _maskRequisite(m && m.requisite_value);
}
...
function _renderWithdrawForm(contentEl, methods) {
    ...
    if (!methods.length) {
        const wrap = _payoutDom('div', 'flex flex-col gap-3');
        wrap.appendChild(_payoutDom('div', 'text-center text-gray-500 text-[11px] py-3', 'Нет сохранённых методов получения. Добавьте метод в разделе «Методы получения».'));
        const addBtn = _payoutDom('button', 'w-full bg-primary/10 border border-primary/20 text-primary py-2.5 rounded-xl font-bold text-[10px] uppercase tracking-wider hover:bg-primary/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2');
        addBtn.type = 'button';
        addBtn.appendChild(_payoutDom('span', 'material-symbols-rounded text-sm', 'add'));
        addBtn.appendChild(document.createTextNode('Добавить метод'));
        addBtn.addEventListener('click', () => openReferralMethodsModal());
    ...
    const wrap = _payoutDom('div', 'flex flex-col gap-3');
    wrap.appendChild(_payoutDom('div', 'text-[9px] text-gray-500 uppercase font-bold tracking-widest', 'Метод получения'));
    const list = _payoutDom('div', 'flex flex-col gap-1.5');
    list.id = 'withdraw-method-list';
    methods.forEach((m, i) => {
        const sel = i === 0;
        const btn = _payoutDom('button', 'flex items-center gap-3 p-3 border rounded-xl transition-all text-left w-full ' + (sel ? 'bg-primary/10 border-primary/30' : 'bg-white/5 border-white/5'));
        btn.type = 'button';
        btn.id = 'wm-' + String(m.id);
        btn.dataset.methodId = String(m.id);
        const iconWrap = _payoutDom('div', 'w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ' + (sel ? 'bg-primary/20' : 'bg-white/10'));
        iconWrap.appendChild(_payoutDom('span', 'material-symbols-rounded text-sm ' + (sel ? 'text-primary' : 'text-gray-400'), _M_ICONS[m.method_type] || 'account_balance'));
        const texts = _payoutDom('div', 'flex-1 min-w-0');
        texts.appendChild(_payoutDom('div', 'text-[10px] font-bold text-white', _payoutMethodLabel(m)));
        texts.appendChild(_payoutDom('div', 'text-[9px] text-gray-500 font-mono truncate', _payoutRequisiteText(m)));
        btn.appendChild(iconWrap);
        btn.appendChild(texts);
        btn.addEventListener('click', () => _selectWithdrawMethod(m.id));
        list.appendChild(btn);
    });
    wrap.appendChild(list);
    const minW = window._referralMinWithdraw || 100;
    const amountBox = _payoutDom('div');
    amountBox.appendChild(_payoutDom('div', 'text-[9px] text-gray-500 uppercase font-bold tracking-widest mb-1.5', 'Сумма (₽) · мин. ' + minW.toFixed(0) + ' ₽'));
    const amountInput = _payoutDom('input', 'w-full bg-black/40 border border-white/10 rounded-xl p-3 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-primary/50 transition-colors');
    amountInput.type = 'number';
    amountInput.id = 'withdraw-amount-input';
    amountInput.min = String(minW);
    amountInput.step = '1';
    amountInput.placeholder = 'Например: ' + minW.toFixed(0);
    amountBox.appendChild(amountInput);
    wrap.appendChild(amountBox);
    const submitBtn = _payoutDom('button', 'w-full bg-white text-black py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider shadow-md hover:bg-gray-100 active:scale-[0.98] transition-all flex items-center justify-center gap-2');
    submitBtn.type = 'button';
    submitBtn.id = 'withdraw-submit-btn';
    submitBtn.appendChild(_payoutDom('span', 'material-symbols-rounded text-sm', 'send'));
    submitBtn.appendChild(document.createTextNode('Отправить заявку'));
    submitBtn.addEventListener('click', () => _submitWithdrawRequest());
...
async function loadWithdrawalRequests() {
    ...
    try {
        ...
        if (!d.ok || !d.withdrawals || !d.withdrawals.length) {
            el.replaceChildren(_payoutDom('div', 'text-center text-[11px] text-gray-500 py-3', 'Нет заявок'));
            return;
        }
        ...
        const wrap = _payoutDom('div', 'flex flex-col gap-2');
        d.withdrawals.forEach(w => {
            ...
            const row = _payoutDom('div', 'glass-card border border-white/10 rounded-xl p-3 flex items-center gap-3');
            const iconWrap = _payoutDom('div', 'w-7 h-7 bg-primary/10 rounded-lg flex items-center justify-center shrink-0');
            iconWrap.appendChild(_payoutDom('span', 'material-symbols-rounded text-primary text-sm', _M_ICONS[w.method_type] || 'payments'));
            const texts = _payoutDom('div', 'flex-1 overflow-hidden');
            const amount = w.amount ? Number(w.amount).toFixed(0) + ' ₽' : '—';
            const methodLabel = _payoutMethodLabel(w);
            texts.appendChild(_payoutDom('div', 'text-[10px] font-bold text-white', amount + (methodLabel ? ' · ' + methodLabel : '')));
            const meta = (w.created_at ? String(w.created_at).slice(0, 10) : '')
                + (w.status === 'rejected' && w.reject_reason ? ' — ' + w.reject_reason : '');
            texts.appendChild(_payoutDom('div', 'text-[9px] text-gray-500 truncate', meta));
            row.appendChild(iconWrap);
            row.appendChild(texts);
            row.appendChild(_payoutDom('span', 'text-[9px] ' + statusColor + ' font-bold uppercase shrink-0', statusLabel));
...
function openTopUpModal() {
    ...
    // Неоплаченный счёт восстанавливаем и досматриваем до конца — так же,
    // как openPaymentModal восстанавливает счёт за ключ.
    const pending = _readPendingTopUp();
    if (pending) {
        window._topUpAmount = pending.amount;
        _renderTopUpWaiting(
            contentEl,
            pending.amount,
            pending.methodName,
...
// Счёт на пополнение переживает перезагрузку Mini App — так же, как счёт
// за ключ (см. pendingPayment / restorePendingPayment). Без этого выход на
// страницу оплаты и возврат обратно терял платёж: окно открывалось на шаге
// ввода суммы, как будто оплаты не было.
const TOPUP_PENDING_KEY = 'pendingTopUp';
const TOPUP_PENDING_TTL_MS = 3600000;

function _rememberPendingTopUp(info) {
    try {
        localStorage.setItem(TOPUP_PENDING_KEY, JSON.stringify({ ...info, timestamp: Date.now() }));
    } catch (e) { /* приватный режим — просто живём без восстановления */ }
}

function _forgetPendingTopUp() {
    try { localStorage.removeItem(TOPUP_PENDING_KEY); } catch (e) { /* см. выше */ }
}

function _readPendingTopUp() {
    try {
        const stored = localStorage.getItem(TOPUP_PENDING_KEY);
        if (!stored) return null;
        const data = JSON.parse(stored);
        if (!data || !data.paymentId || Date.now() - data.timestamp > TOPUP_PENDING_TTL_MS) {
            _forgetPendingTopUp();
            return null;
        }
        return data;
...
// Отслеживание счёта прекращается только на клиенте: счёт у провайдера
// остаётся действующим, и если пользователь всё-таки оплатит уже открытую
// страницу, вебхук обработает платёж и баланс пополнится. Поэтому кнопка
// и называется «вернуться к вводу суммы», а не «отменить счёт» — иначе
// она обещала бы отмену, которой здесь нет.
function _stopTrackingTopUp() {
    _stopStatusPolling();
    _forgetPendingTopUp();
    window._topUpPaymentId = null;
    window._topUpPaymentUrl = null;
    const contentEl = document.getElementById('action-modal-content');
    if (contentEl) _renderTopUpAmountStep(contentEl);
}
...
function _renderTopUpWaiting(contentEl, amount, methodName, paymentId, paymentUrl, isStars, methodId) {
    ...
    if (paymentId) {
        _rememberPendingTopUp({ paymentId, amount, methodName, methodId, isStars, paymentUrl });
    }
    contentEl.innerHTML = `
        <div class="flex flex-col items-center gap-4 py-4 text-center">
            <div class="w-14 h-14 rounded-2xl bg-primary/10 border border-primary/20 flex items-center justify-center">
                <span id="topup-waiting-icon" class="material-symbols-rounded text-primary text-2xl animate-pulse">${isStars ? 'star' : 'hourglass_top'}</span>
            </div>
            <div>
                <div id="topup-waiting-title" class="text-sm font-bold text-white mb-1">Ожидаем оплату</div>
                <div id="topup-waiting-desc" class="text-[11px] text-gray-400 leading-relaxed">
                    ${isStars
    ...
    `;
    if (paymentId) _startTopUpPolling(paymentId, amount);
    if (showVerify) {
        setTimeout(() => { verifyPlategaTopUp({ silentPending: true }); }, 4000);
    }
}

function _markTopUpProcessing(message) {
    const titleEl = document.getElementById('topup-waiting-title');
    const descEl = document.getElementById('topup-waiting-desc');
    const iconEl = document.getElementById('topup-waiting-icon');
    if (titleEl) titleEl.textContent = 'Оплата получена';
    if (descEl) descEl.textContent = message || 'Зачисляем на баланс — это займёт несколько секунд.';
    if (iconEl) iconEl.textContent = 'autorenew';
}

function _renderTopUpSuccess(amount, data) {
    const contentEl = document.getElementById('action-modal-content');
    if (!contentEl) return;
    const credited = Number(amount);
    const balance = data && data.balance != null ? Number(data.balance) : null;
    contentEl.innerHTML = `
        <div class="flex flex-col items-center gap-4 py-4 text-center">
            <div class="relative success-pop">
                <div class="absolute inset-[-8px] bg-emerald-500/20 blur-xl rounded-full"></div>
                <div class="w-14 h-14 bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-2xl flex items-center justify-center shadow-[0_8px_30px_rgba(16,185,129,0.3)] relative z-10 rotate-3">
                    <span class="material-symbols-rounded text-2xl text-white">check</span>
                </div>
            </div>
            <div class="space-y-0.5">
                <div class="text-sm font-bold text-white">Баланс пополнен</div>
                <div class="text-[11px] text-gray-500 font-medium">Оплата подтверждена и зачислена</div>
            </div>
            <div class="w-full bg-[#141414] border border-white/[0.04] rounded-2xl p-4 space-y-3 text-left">
                <div class="flex items-center justify-between">
                    <span class="text-[9px] text-gray-600 font-bold uppercase tracking-[0.15em]">Зачислено</span>
                    <span class="text-[11px] text-emerald-400 font-bold bg-emerald-500/10 px-2.5 py-1 rounded-lg border border-emerald-500/10">${isNaN(credited) ? '—' : '+' + credited.toFixed(2) + ' ₽'}</span>
                </div>
                <div class="flex items-center justify-between">
...
function _startTopUpPolling(paymentId, amount) {
    startStatusPolling(paymentId, async (data) => {
        // Результат показываем до обновления остального интерфейса: иначе
        // ошибка в одном из обновлений оставляла окно на «ожидаем оплату».
        _forgetPendingTopUp();
...
async function _loadReferralPayoutMethods(contentEl) {
    ...
    try {
        ...
        const wrap = _payoutDom('div', 'flex flex-col gap-2');
        if (!methods.length) {
            wrap.appendChild(_payoutDom('div', 'text-center text-gray-500 text-[11px] py-3', 'Пока нет сохранённых способов получения. Нажмите «Добавить способ».'));
        }
        methods.forEach(m => {
            const row = _payoutDom('div', 'flex items-center gap-3 p-3 bg-white/5 border border-white/5 rounded-xl');
            const iconWrap = _payoutDom('div', 'w-8 h-8 bg-primary/10 rounded-lg flex items-center justify-center shrink-0');
            iconWrap.appendChild(_payoutDom('span', 'material-symbols-rounded text-primary text-sm', _M_ICONS[m.method_type] || 'account_balance'));
            const texts = _payoutDom('div', 'flex-1 min-w-0');
            texts.appendChild(_payoutDom('div', 'text-[10px] font-bold text-white', _payoutMethodLabel(m)));
            texts.appendChild(_payoutDom('div', 'text-[9px] text-gray-500 font-mono truncate', _payoutRequisiteText(m)));
            const delBtn = _payoutDom('button', 'w-7 h-7 flex items-center justify-center rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 active:scale-90 transition-all shrink-0');
            delBtn.type = 'button';
            delBtn.appendChild(_payoutDom('span', 'material-symbols-rounded text-sm', 'delete'));
            delBtn.addEventListener('click', () => _deletePayoutMethod(m.id));
            row.appendChild(iconWrap);
            row.appendChild(texts);
            row.appendChild(delBtn);
            wrap.appendChild(row);
        });
        const addBtn = _payoutDom('button', 'w-full mt-2 bg-primary/10 border border-primary/20 text-primary py-2.5 rounded-xl font-bold text-[10px] uppercase tracking-wider hover:bg-primary/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2');
        addBtn.type = 'button';
        addBtn.appendChild(_payoutDom('span', 'material-symbols-rounded text-sm', 'add'));
        addBtn.appendChild(document.createTextNode('Добавить метод'));
        addBtn.addEventListener('click', () => _addReferralPayoutMethod());
        wrap.appendChild(addBtn);
        contentEl.replaceChildren(wrap);
    } catch (e) {
        contentEl.innerHTML = '<div class="text-center text-red-400 py-3 text-xs">Ошибка сети</div>';
...
function _profileDom(tag, className, text) {
    const el = document.createElement(tag);
    if (className) el.className = className;
    if (text != null) el.textContent = text;
    return el;
...
function _profileIconBtn(className, iconName, label, onClick) {
    const btn = _profileDom('button', className);
    btn.type = 'button';
    const icon = _profileDom('span', 'material-symbols-rounded text-sm', iconName);
...
function _renderProfileMain(contentEl, info) {
    const wrap = _profileDom('div', 'flex flex-col gap-3');
    const box = _profileDom('div', 'bg-black/20 rounded-xl p-3 border border-white/5');
    box.appendChild(_profileDom('div', 'text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5', 'Email'));
    box.appendChild(_profileDom('div', 'text-[11px] text-gray-200 font-mono truncate', info.auth_email || '—'));
    wrap.appendChild(box);

    const modalContent = () => document.getElementById('action-modal-content');
    if (info.pending_email) {
        const banner = _profileDom('div', 'flex items-start gap-2 bg-amber-500/8 border border-amber-500/20 rounded-xl px-3 py-2.5');
        banner.appendChild(_profileDom('span', 'material-symbols-rounded text-amber-400 text-sm shrink-0 mt-0.5', 'mail'));
        const msg = _profileDom('span', 'text-[10px] text-amber-200/80 leading-relaxed');
        msg.appendChild(document.createTextNode('Ожидает подтверждения смена email на '));
        const pending = _profileDom('b', 'text-amber-200');
        pending.textContent = info.pending_email;
        msg.appendChild(pending);
        msg.appendChild(document.createTextNode('.'));
        banner.appendChild(msg);
        wrap.appendChild(banner);
        wrap.appendChild(_profileIconBtn(
            'w-full bg-amber-500 hover:bg-amber-600 text-black py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider active:scale-[0.98] transition-all flex items-center justify-center gap-2',
            'pin',
            'Ввести код подтверждения',
            () => _renderProfileVerifyEmailCode(modalContent(), info.pending_email)
        ));
        ...
        wrap.appendChild(_profileIconBtn(
            'w-full bg-white/5 text-white py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider hover:bg-white/10 active:scale-[0.98] transition-all flex items-center justify-center gap-2 border border-white/5',
            'lock_reset',
            'Изменить пароль',
            () => _renderProfileChangePassword(modalContent())
        ));
        wrap.appendChild(_profileIconBtn(
            'w-full bg-white/5 text-white py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider hover:bg-white/10 active:scale-[0.98] transition-all flex items-center justify-center gap-2 border border-white/5',
            'alternate_email',
            'Изменить email',
            () => _renderProfileChangeEmailRequest(modalContent())
        ));
    }
    contentEl.replaceChildren(wrap);
}
...
function _renderProfileVerifyEmailCode(contentEl, pendingEmail) {
    const wrap = _profileDom('div', 'flex flex-col gap-3');
    const hint = _profileDom('div', 'text-[10px] text-gray-400 leading-relaxed');
    hint.appendChild(document.createTextNode('Код отправлен на '));
    const emailEl = _profileDom('b', 'text-white');
    emailEl.textContent = pendingEmail || '';
    hint.appendChild(emailEl);
    hint.appendChild(document.createTextNode('. Введите его ниже, чтобы подтвердить смену email.'));
    wrap.appendChild(hint);

    const codeInput = _profileDom('input', 'w-full bg-black/40 border border-white/10 rounded-xl p-3 text-center text-lg tracking-[0.3em] font-mono text-white placeholder-gray-600 focus:outline-none focus:border-primary/50 transition-colors');
    codeInput.type = 'text';
    codeInput.id = 'profile-email-code';
    codeInput.maxLength = 6;
    codeInput.inputMode = 'numeric';
    codeInput.placeholder = '000000';
    wrap.appendChild(codeInput);

    const verifyBtn = _profileDom('button', 'w-full bg-white text-black py-3 rounded-xl font-bold text-[11px] uppercase tracking-wider shadow-md hover:bg-gray-100 active:scale-[0.98] transition-all flex items-center justify-center gap-2');
...
// second inline script had also been parsed. Calling it here (after all
// functions in this file) preserves that order.

...
def _get_payment_methods() -> dict:
    ...
    rollypay_api_key = get_setting('rollypay_api_key')
    rollypay_terminal_id = get_setting('rollypay_terminal_id')
    rollypay_enabled = bool(rollypay_api_key and rollypay_terminal_id)
...
def get_user_router() -> Router:
    ...
    def _rollypay_is_enabled() -> bool:
        return bool(
            ...
            and (get_setting("rollypay_terminal_id") or "").strip()
        )

    async def _create_rollypay_payment_link(amount_rub, payment_id: str, description: str):
        ...
        rollypay = RollyPayAPI(api_key, terminal_id)
        return await rollypay.create_payment(
    ...
    async def check_rollypay_payment_handler(callback: types.CallbackQuery, bot: Bot):
        ...
        try:
            ...
            logger.error(f"RollyPay manual check: failed to read pending metadata for {pid}: {e}")

        if not pending_meta:
            await callback.answer("❌ Платёж не найден. Попробуйте позже.", show_alert=True)
            return

        provider_payment_id = str(pending_meta.get("rollypay_payment_id") or "").strip()
        if not provider_payment_id:
            await callback.answer("⏳ Платеж ещё не подтверждён. Попробуйте позже.", show_alert=True)
            return

        if not _rollypay_is_enabled():
            await callback.answer("⚠️ RollyPay не настроен. Обратитесь к администратору.", show_alert=True)
            return
        ...
        remote = await RollyPayAPI(api_key, terminal_id, signing_secret).get_payment(provider_payment_id)
        if not remote:
            await callback.answer("⏳ Не удалось проверить статус. Попробуйте позже.", show_alert=True)
            return

        remote_status = str(
            remote.get("status") or remote.get("event_type") or remote.get("payment_status") or ""
        ).lower()
        paid_statuses = {"paid", "succeeded", "success", "completed", "payment.paid"}
        failed_statuses = {"failed", "canceled", "cancelled", "expired", "refunded", "chargeback"}
        if remote_status in failed_statuses:
            await callback.answer("❌ Платёж не прошёл.", show_alert=True)
            return
        if remote_status not in paid_statuses:
            await callback.answer("⏳ Платеж ещё не подтверждён. Попробуйте позже.", show_alert=True)
            return
